...
bool HeroChoice::init()
{
	...
	/*设置管卡选择按钮*/
	const char* chooseImage[3] = { "LeftArrow1.jpg", "RightArrow1.jpg",
		"MidArrow1.png" };
	float fSettings[4][4] = {
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		auto pPaddle = Paddle::paddleWithTexture(paddleTexture, evts[i], 1);
...
Scene * HeroChoice::scene(int _model)
{
...
void HeroChoice::clickDownAction(typeNum _type)
{
	if (_type == typeNum::mid) {
		Scene* scene = ChoiceScene::scene(nowHero, m_model);
